            ...
            link: function ($scope, $element, $attrs, $ngModelCtrl) {
                ...
                $scope.getButtonText = function () {
                    ...
                    if ($scope.selectedOptions && $scope.selectedOptions.length > 1) {
                        ...
                        if (totalSelected === 0) {
                            return $scope.labels && $scope.labels.select ? $scope.labels.select : 'Select ' + $scope.name || '';
                        } else {
                            return totalSelected + ' ' + ($scope.labels && $scope.labels.itemsSelected ? $scope.labels.itemsSelected : (angular.lowercase($scope.name) || '') + ' selected');
                        }
                    } else {
                        return $scope.labels && $scope.labels.select ? $scope.labels.select : 'Select ' + $scope.name || '';
                    }
...
angular.module("multiselect.html", []).run(["$templateCache", function ($templateCache) {
    $templateCache.put("multiselect.html",
      ...
      "    <button type=\"button\" class=\"form-control\" ng-click=\"toggleDropdown()\" ng-disabled=\"disabled\">\n" +
